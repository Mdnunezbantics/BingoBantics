            ...
            if num != "X":
                ...
                for image in xmldoc.getElementsByTagName('image'):
                    if image.getAttribute('id') == str(image_id):
                        ...
                        # image.setAttribute('xlink:href', str(png_url))
    d_fijos = {
        "%var_tira1%": completa_numero(5, tira1),
        "%var_tira2%": completa_numero(5, tira2),
...
def multi_svgs_a_1_pdf(pdf_file_name, output_path, svgs_dir, png_path):
    ...
    # temp_png_original = os.path.join(temp_svg_original, ntpath.basename(png_path))
    ...
    # temp_png_testigo = os.path.join(temp_svg_testigo, ntpath.basename(png_path))
