...
def hacer_pdfs(base_dir, salida_dir_name, pdfs_name, preparar_testigo=False):
    ...
    for svg in svg_list:
        ...
        if hoja == datos_variables.hojas_por_pdf:
            print("Preparamos PDF: hasta la hoja:", str(pdf * int(datos_variables.hojas_por_pdf)))
            pdf_file_name = pdfs_name + completa_numero(5, pdf) + ".pdf"
            multi_svgs_a_1_pdf(pdf_file_name, output_dir, temp_svg, hacer_testigo=preparar_testigo)
